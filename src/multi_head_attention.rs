...
pub enum AttentionError {
    #[error("Cache leer – zunächst `clear_cache()` aufrufen")]
    EmptyCache,
}
...
pub struct MultiHeadAttention {
    ...
    // Gewichtsmatrizen
    w_qkv: Array2<f32>,  // (embed, 3*embed)
    w_o:   Array2<f32>,  // (embed, embed)

    // Key/Value-Cache (optional genutzt)
...
impl MultiHeadAttention {
    pub fn new(i_embed: usize, i_heads: usize, f_dropout: f32) -> Self {
        ...
        assert!(i_embed % i_heads == 0, "i_embed muss durch i_heads teilbar sein");
    ...
    pub fn forward(&mut self, m_x: &Array2<f32>, i_step: usize) -> Array2<f32> {
        ...
        let mut m_q = m_q_view.to_owned(); // (seq, embed)
        let mut m_k = m_k_view.to_owned(); // (seq, embed)
        let     m_v = m_v_view.to_owned(); // (seq, embed)

        // 2) RoPE
        apply_rope(m_q.view_mut(), m_k.view_mut(), i_step);
        ...
        // 4) pro Head auftrennen
        let q_heads = self.split_heads_vec(&m_q); // Vec<(seq, head_dim)>
        ...
        // 6) Heads wieder zusammenfügen: (seq, embed)
        let m_concat = self.merge_heads_vec(&ctx_per_head);

        // 7) Projektion + einfache Regulierung
    ...
    // Hilfsfunktionen: Heads trennen / zusammenfügen
...
// einfache Zeilen-Softmax: (seq, n) -> (seq, n)
